class Brain {
  PVector[] directions;//series of vectors which get the square to the goal (hopefully)
  int step = 0;
  ...
  void mutate() {
    float mutationRate = 0.1;//chance that any vector in directions gets changed
    for (int i =0; i< directions.length; i++) {
      float rand = random(1);
